...
> + `src/components/instagramGallery/` + `src/constants/instagramGallery.js`).

### The `features/` ↔ `components/` split
For most domains (auth, cart, checkout, productDetails, wishlist, orders,
profile, navbar, footer, hero, faq, instagramGallery, etc.) there is a folder
of the same name under **both** `src/features/` and `src/components/`:
...
| Change the account dropdown (logged-in menu, guest menu) | `src/components/accountDropdown/` + `src/features/accountDropdown/` |
| Change the footer (links, newsletter, accordion) | `src/components/footer/` + `src/features/footer/` |
| Change the homepage hero slider | `src/components/hero/` + `src/features/hero/` (`slider.js` = drag/autoplay, `timeline.js` = autoplay timer, `animations.js` = enter/zoom transitions) + `src/constants/heroSlides.js` |
| Change the homepage showcase (trending/new-arrival tabs) | `src/components/showcase/` + `src/features/showcase/` + `src/constants/showcaseProducts.js` (fallback data) |
| Change the homepage Instagram gallery | `src/components/instagramGallery/` + `src/features/instagramGallery/` + `src/constants/instagramGallery.js` |
| Change the homepage marquee / scrolling banner | `src/components/marquee/` + `src/features/marquee/` |
| Change the top announcement bar | `src/components/announcement/` + `src/features/announcement/` |
| Change "why choose us" section | `src/components/whyChooseUs/` + `src/features/whyChooseUs/` + `src/constants/whyChooseUs.js` |
| Change the craftsmanship/process section | `src/components/craftsmanship/` + `src/features/craftsmanship/` |
| Change the homepage collections grid | `src/components/collections/` + `src/features/collections/` |
...
| Change "Quick Add" (pick-a-size add-to-cart modal from product cards) | `src/features/quickAdd/` + `src/components/quickAdd/` |
| Change checkout flow (address, payment methods, order summary/confirmation) | `src/features/checkout/` (`addressPanel.js`, `paymentPanel.js`, `orderPanel.js`, `checkoutPageInit.js`) + `src/components/checkout/` + `src/pages/checkoutPage.js` |
...
| Change hardcoded/fallback content (nav links, hero slides, Instagram gallery picks, payment method list, "why choose us" copy, fallback showcase products) | `src/constants/` |
...
- **`index.js` files are barrels**: most `components/<domain>/index.js`
  and some `features/<domain>/index.js` files exist only to re-export
  from sibling files in the same folder (e.g. `export { createFooter }
  from "./footer.js"`) — open the sibling file for actual logic.
...
behavior/state — see the naming convention above. Barrel `index.js` files
that only re-export siblings are marked "barrel" rather than repeated.
...
| `homePage.js` | `initHomePage()` — boots the homepage: hero, showcase, Instagram gallery, marquee, craftsmanship, why-choose-us, home FAQ, collections, social proof toasts. |
...
| `toast.js` | `showToast({...})` — low-level trigger used by feature code to fire a toast notification (renders via `src/components/toast/`). |
...
| `navigation.js` | `NAVIGATION`, `FOOTER_NAVIGATION`, `SOCIAL_LINKS` — navbar links, footer link groups, and social icon links. |
| `paymentMethods.js` | Static list of payment methods offered at checkout. |
...
| File | What it does |
|---|---|
| `accountDropdown/index.js` | barrel — re-exports `initAccountDropdown`. |
...
| `announcement/index.js` | `getActiveMarquees()`, `initAnnouncementBar(container)` — fetches and renders the top announcement/marquee bar. |
| `auth/index.js` | barrel — re-exports everything from `authState.js`, `authGuard.js`, `authModal.js`, `authTriggers.js`, `guestEngagement.js`. |
...
| `collections/index.js` | `initCollections()` — wires the homepage collections grid. |
| `collections/renderCollections.js` | `renderCollections()` — fetches/renders the collection cards. |
| `contact/contact.js` | `initContact()` — wires the contact form submit flow. |
...
| `craftsmanship/index.js` | `initCraftsmanship()` — wires the homepage craftsmanship/process section. |
| `customizeJewellery/index.js` | `initCustomizeJewellery()` — wires the floating "Customize Jewellery" button + modal together. |
...
| `faqPage/index.js` | `initFAQ()` — wires the dedicated `/faq` page (accordion + category filter). |
...
| `hero/index.js` | `initHero()` — wires the homepage hero (slides + slider + timeline + animations together). |
...
| `instagramGallery/index.js` | `initInstagramGallery()` — wires the homepage Instagram gallery section. |
| `instagramGallery/renderInstagramGallery.js` | `renderInstagramGallery()` — renders the gallery tiles from `src/constants/instagramGallery.js` into the section built by `src/components/instagramGallery/`. |
| `marquee/index.js` | `initMarquee()` — wires the scrolling marquee/banner strip. |
| `navbar/index.js` | `initNavbar()` — top-level navbar wiring (ties together active-link, scroll, mobile drawer, search overlay). |
...
| `productDetails/index.js` | `initProductDetailsPage()` — top-level PDP wiring; imports and calls the other `productDetails/*` init functions below. |
...
| `productDetails/query.js` | `getProductId()`, `getProductSlug()` — reads the product identifier out of the current URL. |
...
| `productDetails/state.js` | `productState` + setters (`setProduct`, `applyVariantToProduct`, `setQuantity`, `setActiveImage`, `setActiveTab`, `setSelectedSize`, `setSelectedVariant`) — the single in-memory state object the whole PDP reads/writes. |
...
| `products/index.js` | `initProductsPage()` — top-level wiring for the product listing/category page. |
...
| `quickAdd/index.js` | `initQuickAdd()` — wires the "Quick Add" size-picker modal triggered from product cards. |
| `scrollToTop/index.js` | `initScrollToTopButton()` — show/hide + click-to-top behavior. |
| `showcase/index.js` | `initShowcase()` — wires the homepage showcase section (tabs + carousel + data load together). |
...
| `socialProof/index.js` | `initSocialProofToasts()` — drives the rotating "X just explored Y" toast widget (timing/rotation logic). |
| `toast/index.js` | `initToast()` — global toast container init, listens for toast-trigger events from `src/utils/toast.js`. |
| `whatsapp/index.js` | `initFloatingWhatsAppButton()` — wires the floating WhatsApp contact button. |
| `whyChooseUs/index.js` | barrel — re-exports `renderWhyChooseUs`. |
| `whyChooseUs/renderFeatures.js` | `renderWhyChooseUs()` — renders the "why choose us" feature cards from `src/constants/whyChooseUs.js`. |
...
| File | What it does |
|---|---|
| `accountDropdown/index.js` | barrel — re-exports `createAccountDropdown`, `createGuestMenu`, `createUserMenu`. |
| `accountDropdown/dropdown.js` | `createAccountDropdown(user)` — builds the navbar account dropdown shell. |
...
| `announcement/announcementBar.js` | `createAnnouncementBar(marquees)` — markup for the top announcement/marquee bar. |
| `auth/index.js` | barrel — re-exports every `create*` function below in this folder. |
...
| `auth/forgotPasswordPage.js` | `createForgotPasswordPage()` — full forgot-password page (layout + form). |
| `auth/loginForm.js` | `createLoginForm()` — the login form markup. |
| `auth/page.js` | `createLoginPage()` — full login page (layout + form). |
| `auth/registerForm.js` | `createRegisterForm()` — the register form markup. |
| `auth/registerPage.js` | `createRegisterPage()` — full register page (layout + form). |
...
| `collections/collectionCard.js` | `createCollectionCard(collection)` — a single homepage collection card. |
| `contact/contactForm.js` | `createContactForm(contactInfo)` — the contact page form + info block. |
| `craftsmanship/craftsmanshipSection.js` | `createCraftsmanshipSection()` — homepage craftsmanship/process section markup. |
| `customizeJewellery/index.js` | barrel — re-exports `createCustomizeJewelleryButton`, `createCustomizeJewelleryModal`. |
| `customizeJewellery/customizeProducts.js` | `createCustomizeProductCard(...)` — product card used inside the customize modal's picker. |
...
| `customizeJewellery/modal.js` | `createCustomizeJewelleryModal()` — the customize modal shell. |
| `faq/faqCard.js` | `createFaqCard(faq)` — a single FAQ accordion item (used by both `faqPage` and `homeFaq`). |
| `footer/index.js` | barrel — re-exports `createFooter`, `createFooterLinks`, `createCopyright`, `initFooterAccordion`. |
...
| `footer/footerAccordion.js` | `initFooterAccordion()` — mobile footer accordion expand/collapse (note: behavior living in a `components/` file, an exception to the usual split). |
...
| `hero/index.js` | barrel — re-exports `createHero`, `createHeroSlides`, `createHeroIndicators`. |
...
| `instagramGallery/instagramGallerySection.js` | `createInstagramGallerySection()` — homepage Instagram gallery section shell. |
| `instagramGallery/instagramGalleryTile.js` | `createInstagramGalleryTile(entry)` — a single gallery image tile. |
| `marquee/marqueeSection.js` | `createMarqueeSection(items)` — the scrolling marquee strip markup. |
| `navbar/index.js` | barrel — re-exports `createNavbar`, `createDesktopNav`, `createMobileNav`, `createSearchOverlay`. |
| `navbar/desktopNav.js` | `createDesktopNav(...)` — desktop nav links markup. |
| `navbar/mobileNav.js` | `createMobileNav()`, `refreshMobileAccount()`, `initMobileDrawer()` — mobile nav drawer markup **and** its drawer open/close wiring (an exception to the usual split — behavior lives here too). |
| `navbar/navbar.js` | `createNavbar(...)` — overall navbar shell (assembles desktop/mobile nav, account dropdown, search). |
| `navbar/searchOverlay.js` | `createSearchOverlay()` — full-screen search overlay markup. |
...
| `quickAdd/quickAddModal.js` | `createQuickAddModal()` — the "Quick Add" size-picker modal markup. |
| `scrollToTop/index.js` | barrel — re-exports `createScrollToTopButton`. |
| `scrollToTop/scrollToTopButton.js` | `createScrollToTopButton()` — the scroll-to-top button markup. |
...
| `socialProof/index.js` | barrel — re-exports the social-proof widget builders. |
...
| `toast/index.js` | barrel — re-exports `createToastContainer`. |
| `toast/toast.js` | `createToastContainer()` — the toast notification container/markup. |
| `whatsapp/index.js` | barrel — re-exports `createFloatingWhatsAppButton`. |
| `whatsapp/floatingButton.js` | `createFloatingWhatsAppButton(...)` — the floating WhatsApp button markup. |
| `whyChooseUs/featureCard.js` | `createFeatureCard(feature)` — a single "why choose us" feature card. |
